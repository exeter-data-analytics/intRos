...
4. The root of your working directory now starts wherever the Project's `.Rproj` file is — you can test this by running `getwd()` in a new script. While this filepath which would normally start at the top of your disk drive, it should now start right in the project folder). So no more writing — `C:\extremely\long\filepath\that\only\I\have` every time (**BUT** see next section on the `here` package for making this even better).

## Best practices for structuring your project folder
...
#| echo: false
#| fig-align: left
#| fig-cap: Example of what your sub-folders might look like inside your Project folder (adapted from Martin Chan's [beginners guide to using R Studio projects](https://martinctc.github.io/blog/rstudio-projects-and-working-directories-a-beginner's-guide/))
library(tidyverse)
library(magick)

data <- tibble(name = c("main", "main", "main", "sub1", "sub1", "sub2", "sub2", "sub3", "sub3"), 
               x = c(1, 1, 1.7, 1, 1.66, 1, 1.7, 1, 1.7), 
...
ggplot(data = data) +
  ...
  geom_text(aes(x = 1.9, y = y[5]), label = ".Rproj file", hjust = 0, size = 6) +
  geom_text(aes(x = 1.9, y = y[7]), label = "raw_data", hjust = 0, size = 6) +
  geom_text(aes(x = 1.9, y = y[9]), label = "code", hjust = 0, size = 6) +
  geom_text(aes(x = 1.9, y = y[3]), label = "outputs", hjust = 0, size = 6) +
...
R Studio projects go a long way to making shorter filepaths for our code, but we can make things even better and more reproducible by  making use of the `here` package. `here` helps make filepaths consistent between different operating systems. For example, while macOS users have filepaths separated by forward slashes (`/`), windows users have filepaths with backward slashes (`\`). This means that even with the shorter filepaths we get when using Projects, custom filepaths like `raw_data/2023.csv` will only work for macOS users, and if you share this fragile code with someone using Windows your code will just throw them errors. 

In comes the `here` package to the rescue — we can use it to make reproducible filepaths that work for everyone! `here` works in a similar way to `file.path`, where we build a path to a file by listing the folders of the path as arguments. For example, if we had some data in a project sub folder of `raw_data` called `2023.csv`, we'd make a reproducible filepath to it by writing `here("raw_data, "2023.csv")`. `here` does some helpful stuff in the background by a) adding our project's root directory (which you can see by just writing `here()` with no file names), and b) starts evaluating from the project up (which `file.path` doesn't). 

When you want to actually use a filepath for say, reading some data with `read_csv`, you just have to provide this simple `here` code wherever you'd be writing a `Users/stephen's/long/winded/filepath/to/a/file`:

```{r}
#| eval: false
#| output: false
library(here) #load here if not already
...
read_csv(file = here("raw_data", "2019.csv")) #read the file in the filepath above with read_csv
```
...
[**intRo to using git, GitHub & R Studio (post coming soon)**](link to be updated)
...
-   [Kirill Müller](https://github.com/krlmlr) (creator of the `here` package) has written a [primer on getting started with it](https://here.r-lib.org/) 

### Acknowledgements

We did not create this content alone! Inspiration, tips, and resources have been borrowed from multiple sources.

-   Martin Chan's [beginners guide to using R Studio projects](https://martinctc.github.io/blog/rstudio-projects-and-working-directories-a-beginner's-guide/)
